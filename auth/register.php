...
if (isset($_POST['daftar'])) {
    ...
    $peran    = 'pengguna';
    $cek = mysqli_query($conn, "SELECT * FROM alif_pengguna WHERE email='$email'");
    if (mysqli_num_rows($cek) > 0) {
        $pesan = "Email sudah digunakan!";
    } else {
        $query = mysqli_query($conn, "INSERT INTO alif_pengguna (nama_pengguna, email, password, peran) VALUES ('$nama', '$email', '$password', '$peran')");
        if ($query) {
            header("Location: login.php?status=terdaftar");
            exit();
        } else {
...
<div class="container d-flex justify-content-center align-items-center" style="min-height: 100vh;">
  <div class="register-card col-md-5 animate__animated animate__fadeInUp">
    <a href="login.php" class="btn btn-outline-secondary back-btn">← Kembali ke Login</a>
    <h3 class="text-center mb-4" style="color:#009688;font-weight:700;">Registrasi Akun Pengguna</h3>
    <?php if ($pesan != '') : ?>
      <div class="alert alert-warning animate__animated animate__shakeX"><?= $pesan ?></div>
    <?php endif; ?>
    <form method="POST">
      <div class="mb-3">
      ...
      <button type="submit" name="daftar" class="btn btn-modern w-100">Daftar</button>
    </form>
    <div class="text-center mt-3">
      <span>Sudah punya akun?</span> <a href="login.php" class="btn btn-outline-success btn-sm ms-1">Login</a>
    </div>
  </div>
</div>
